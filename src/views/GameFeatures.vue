...
const handleConnect = async () => {
    ...
    try {
        // 断开现有的token连接，只断开其他token，保留当前选中token的连接（如果已连接）
        const connectedTokens = Object.keys(tokenStore.wsConnections);
        for (const tokenId of connectedTokens) {
            if (tokenStore.wsConnections[tokenId].status === "connected" && tokenId !== selectedTokenId.value) {
                tokenStore.closeWebSocketConnection(tokenId);
                message.info(`已断开Token ${tokenStore.gameTokens.find(t => t.id === tokenId)?.name} 的连接`);
            }
        }
        
        // 使用tokenStore.selectToken方法处理token选择和连接，确保状态同步
        ...
        if (token) {
            ...
            // 等待连接建立
            setTimeout(() => {
                const status = tokenStore.getWebSocketStatus(selectedTokenId.value);
                if (status === "connected") {
                    message.success("WebSocket 连接成功");
                    // 连接成功后自动初始化游戏数据
                    initializeGameData();
                } else {
                    message.warning(`连接状态: ${status === "connecting" ? "正在连接中..." : "连接失败，请重试"}`);
                }
            }, 2000);
        }
    } catch (error) {
...
const handleDisconnect = () => {
    if (!tokenStore.selectedToken) {
        message.warning("未选择Token");
        return;
    }
    
    const tokenId = tokenStore.selectedToken.id;
    const tokenName = tokenStore.selectedToken.name;
    
    tokenStore.closeWebSocketConnection(tokenId);
    message.info(`已断开Token ${tokenName} 的连接`);
};
...
onMounted(() => {
    // 初始化选中的token ID
    if (tokenStore.selectedToken) {
        selectedTokenId.value = tokenStore.selectedToken.id;
        const status = tokenStore.getWebSocketStatus(tokenStore.selectedToken.id);
        if (status === "connected") {
            // 如果已连接，立即获取初始数据
            initializeGameData();
